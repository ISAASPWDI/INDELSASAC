...
class Producto
{
    ...
    public function agregar($nombre, $descripcion, $marca, $imagen, $detalles)
    {
        try {
            ...
            // Insertar detalles técnicos
            $query = "INSERT INTO detalles_tecnicos (id_producto, detalles) VALUES (?, ?)";
            $stmt = $this->conn->prepare($query);
            $stmt->execute([$producto_id, json_encode($detalles)]);
...
public function editar($id, $nombre, $descripcion, $marca, $imagen, $detalles)
{
    try {
        $this->conn->beginTransaction();
